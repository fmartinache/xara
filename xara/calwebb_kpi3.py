...
class fix_bad_pixels():
    ...
    def step(self,
        ...
        if self.plot:
            ...
            ax[2].set_title('Full frame (fixed)', y=1., pad=-20, bbox=dict(facecolor='white', edgecolor='lightgrey', boxstyle='round'))
...
class recenter_frames():
    ...
    def step(self,
        ...
        if (self.trim == True):
            ...
            if (data.ndim != 3):
                raise NotImplementedError('Trimming is implemented for 3D data cube only')
            ww_max = []
            for i in range(data.shape[0]):
        ...
        if self.plot:
            ...
            ax[0, 0].set_title('Full frame (log-scale)', y=1., pad=-20, bbox=dict(facecolor='white', edgecolor='lightgrey', boxstyle='round'))
            if (data.ndim == 2):
                p01 = ax[0, 1].imshow(np.log10(np.abs(data_recentered)), origin='lower')
            else:
                p01 = ax[0, 1].imshow(np.log10(np.abs(data_recentered[0])), origin='lower')
...
class window_frames():
    ...
    def step(self,
        ...
        if self.plot:
            ...
            plt.colorbar(p1, ax=ax[1])
            cc = plt.Circle((data_windowed.shape[2]//2, data_windowed.shape[1]//2), wrad, color='red', ls='--', fill=False)
            ax[1].add_patch(cc)
            ...
            ax[1].set_title('Full frame (windowed)', y=1., pad=-20, bbox=dict(facecolor='white', edgecolor='lightgrey', boxstyle='round'))
...
class extract_kerphase():
    ...
    def step(self,
        ...
        # Copied from bad pixel cleaning script.
        if (recenter_frames_obj.trim == True):
            if (INSTRUME != 'NIRISS'):
                raise NotImplementedError('Trimming is implemented for NIRISS only')
            if (data.ndim != 3):
                raise NotImplementedError('Trimming is implemented for 3D data cube only')
